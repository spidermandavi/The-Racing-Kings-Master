...
(function initGlobalMenu(){
  ...
  function readProfileStats(){
    ...
    const get=(label)=>{const row=rows.find(r=>r.querySelector('.stat-label')?.textContent?.trim().toLowerCase()===label.toLowerCase());return row?.querySelector('.stat-value')?.textContent?.replace(/[^0-9.-]/g,'')||'';};
    return {games:Number(get('Total Games'))||0,peakRating:Number(get('Peak Rating'))||0};
  }
