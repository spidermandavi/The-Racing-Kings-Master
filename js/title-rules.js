...
// Every page that evaluates or displays title requirements should consume this file.
window.RK_TITLE_RULES = Object.freeze({
  universal: Object.freeze({
    accountAgeDays: 92,
    minNormGames: 10,
    blitzControls: Object.freeze(['3+0', '3+2', '5+0'])
  }),
...
window.RK_TITLE_RULES.isBlitzControl = function(clock) {
  return window.RK_TITLE_RULES.universal.blitzControls.includes(String(clock ?? '').replace(/\s+/g,''));
};
window.RK_TITLE_RULES.get = function(code) {
  return window.RK_TITLE_RULES.titles[String(code || '').trim().toUpperCase()] || null;
};
